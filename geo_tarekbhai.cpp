...
{
    double x, y;
    PT() {} PT(double x, double y) : x(x), y(y) {} PT(const PT &p) : x(p.x), y(p.y) {} PT operator + (const PT &p) const
    {
...
/** Before calling this, check whether the line segments are parallel or not.Determines whether the line segment between 'a' and 'b' intersects with the line segment between 'c' and 'd'*/
bool SegmentsIntersect(PT a, PT b, PT c, PT d)
{
    if(LinesCollinear(a, b, c, d))
    {
        if(dist2D_sqr(a, c)<EPS || dist2D_sqr(a, d)<EPS || dist2D_sqr(b, c)<EPS || dist2D_sqr(b, d)<EPS)
            return true;
    ...
    { 
       ...
       //   printf("%lf %lf\n",polygon[i].x,polygon[i].y);
        ...
        { 
            ...
            // otherwise false 
            if (LinesCollinear(polygon[i],polygon[next],p,extreme)) 
            return PointOnSegment(polygon[i], p, polygon[next]); 
            //printf("%lf %lf\n",polygon[i].x,polygon[i].y);
            count++; 
        } 
        i = next; 
...
bool PointOnBoundaryOfPolygon(const vector<PT>p, PT q)
{
    ...
    {
        if(PointOnSegment(q, p[j], p[i]))
            return true;
    }
    return false;
}
/** Determines whether a point 'q' is in a possibly non-convex polygon or not.Returns '1' for strictly interior points, '0' for strictly exterior points, and '0' or '1' for the remaining points.*/
